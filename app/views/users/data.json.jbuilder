  ...
  json.array!(@users) do |user|
    ...
    if @current_user.role_ids.include?(686800945) && user.role_ids.any? { |i| [160943002, 6868009].include? i }
      if user.is_active?
        ...
        user_policy = "<span class ='mr5'><a class='btn btn-primary btn-xs' href='/authorization/user_policies/add_policy?id=" + user.id.to_s + "' data-remote='true' data-toggle='modal' data-target='#user-policy-modal' disabled='disabled'>Policy</a></span>"

        reset_user_password = "<span class ='mr5'><a class='btn btn-primary btn-xs' href='/users/reset_password?user_id=" + user.id.to_s + "' data-remote='true' data-toggle='modal' data-target='#user-modal' disabled='disabled'>Reset Password</a></span>"
      ...
      if user.is_active?
        ...
        link_facilities = "<span><a class='btn btn-success btn-xs' style='margin: 4px 0px;padding-right: 25px;' href='/users/link_unlink_multiple_facilities?user_id=" + user.id.to_s + "&level=" + params[:level] + "&flag=link' data-remote='true' data-toggle='modal' data-target='#user-modal'><i class='fa fa-link'></i>  Link Facilities</a></span>"
    ...
    json.set! "2", address.join("<br>") + ' / ' + user.try(:mobilenumber).to_s
    json.set! "3", user.try(:email).to_s + " / " + user.try(:username).to_s + "<div style='display: none'>#{user.try(:integration_identifier)}</div>"
    json.set! "4", user.specialty_names.count > 0 ? user.specialty_names.map { |ele| "<div class='row'><div style='margin-left:15px;margin-bottom:-10px;'>#{ele.upcase}</div></div>" }.join("<br>") : '-'
    json.set! "5", user_actions
  end
end
