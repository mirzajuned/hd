json.array!(@inventory_department_items) do |item|
  ...
  when 'medication'
    ...
    json.extract! item.lots.where('stock' => { '$gte' => 1 }, is_deleted: false).first, :batch_no, :price, :mark_up, :mrp, :list_price, :vendor, :expiry
  when 'stockable'
    json.extract! item, :inventory_capacity, :threshold, :stock
    json.extract! item.lots.where('stock' => { '$gte' => 1 }, is_deleted: false).first, :batch_no, :price, :mark_up, :mrp, :list_price, :vendor, :expiry, :warranty_expiry
  when 'implant'
    json.extract! item, :inventory_capacity, :threshold, :stock
    json.extract! item.lots.where('stock' => { '$gte' => 1 }, is_deleted: false).first, :batch_no, :price, :mark_up, :mrp, :list_price, :vendor, :expiry, :warranty_expiry
  when 'consumable'
