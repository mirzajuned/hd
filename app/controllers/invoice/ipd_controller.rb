class Invoice::IpdController < Invoice::InvoicesController
  ...
  def new
    ...
    @invoice.admission_id = @admission.id.to_s
    @facility_id = @admission.facility_id
    @view_js_erb = '/invoice/ipd/new'
  ...
  def create
    @number_format = CommonHelper.get_number_format(current_facility, current_organisation)
    @invoice = Invoice::Ipd.new(ipd_invoice_params)
    if @invoice.save(validate: false)
      # generate all the sequences for invoice
      bill_number = SequenceManagers::GenerateSequenceService.call('invoice', @invoice.id.to_s, {organisation_id: current_organisation['_id'].to_s, facility_id: current_facility['_id'].to_s, region_id: current_facility['region_master_id'].to_s, department_id: '486546481'})
      @invoice.update(bill_number: bill_number)

      @invoice.record_histories.create(user_id: current_user.id, action: 'C', actiontime: Time.current, template_type: 'Invoice::Ipd')
      ...
      @invoice.advance_payment_breakups.each do |advance_payment_breakup|
        ...
        advance_history = { advance_payment_breakup_id: advance_payment_breakup.id.to_s, invoice_id: @invoice.id.to_s, invoice_type: 'IPD', type: 'Adjusted', user_name: current_user.fullname, amount: advance_payment_breakup.amount, event_time: Time.current.strftime('%I:%M %p, %d/%m/%Y'), bill_number: @invoice.bkp_bill_number }
      ...
      @invoice.services&.each do |serv|
        ...
        if service_list.nil?
          @service_list = Invoice::ServiceList.create(service_name: serv.name, facility_id: @invoice.facility_id, organisation_id: current_user.organisation_id, ipdsum: serv.service_total.to_f, use_count_ipd: 1)
        else
          service_list.update_attributes(ipdsum: (serv.service_total.to_f + service_list.ipdsum.to_f), use_count_ipd: (service_list.use_count_ipd.to_i + 1))
        end
        ...
        serv.items.each do |item|
          ...
          if item_list.nil?
            @item_list = Invoice::ItemList.create(item_name: item.description, facility_id: @invoice.facility_id, organisation_id: current_user.organisation_id, ipdsum: serv.service_total.to_f, use_count_ipd: 1)
          else
            item_list.update_attributes(ipdsum: (item.price.to_f + item_list.ipdsum.to_f), use_count_ipd: (item_list.use_count_ipd.to_i + 1))
          end
        end
      end
      ...
      unless bill_of_materials.empty?
        bill_of_materials.each do |bom|
          ...
          bill_of_material.update(ipd_invoice_id: @invoice.id, is_billed: true, ipd_invoice_date: Date.current,
                                  ipd_invoice_time: Time.current)
        end
      end

      # TaxCalculation
      TaxCalculationJob.perform_later(@invoice.id.to_s, 'tax_collected_details') if @invoice.tax_enabled
      Analytics::Financial::FinancialJob.perform_later(@invoice.id.to_s, 'IPD')
      ...
      Patients::Summary::TimelineWorker.call(event_name: 'IPD_INVOICE', sub_event_name: 'CREATED', invoice_id: @invoice.id, user_id: current_user.id, user_name: current_user.fullname, encounter_type: 'IPD', is_draft: @invoice.is_draft)
    end
  end

  def show
    @number_format = CommonHelper.get_number_format(current_facility, current_organisation)
    @invoice = Invoice::Ipd.find_by(id: params[:id])
    @adjusted_amount = @invoice.advance_payment
    @patient = Patient.find_by(id: @invoice.patient_id)
    @admission = Admission.find_by(id: @invoice.admission_id)
  ...
  def edit
    ...
    options = { facility_id: facility_id, specialty_id: @invoice.specialty_id, department_id: '486546481' }
    ...
    @bill_of_materials = Inpatient::BillOfMaterial.where(patient_id: @patient.id, is_canceled: false)
    @edit_js_erb = '/invoice/ipd/edit'
    fetch_assessment_data
    super
  end
  ...
  def update
    ...
    # generate all the sequences for invoice
    SequenceManagers::GenerateSequenceService.call('invoice', @invoice.id.to_s, 'bill_number', {organisation_id: current_organisation['_id'].to_s, facility_id: current_facility['_id'].to_s, region_id: current_facility['region_master_id'].to_s, department_id: '486546481'}) if @invoice.bill_number.nil?

    @invoice.services&.each do |serv|
      OfferManagerJobs::ManageOfferCountJob.perform_later(serv.offer_manager_id.to_s) if serv.offer_manager_id.present?
      service_list = Invoice::ServiceList.find_by(service_name: serv.name, facility_id: @invoice.facility_id)
      service_list&.update_attributes(ipdsum: (service_list.ipdsum.to_f - serv.service_total.to_f), use_count_ipd: (service_list.use_count_ipd.to_i - 1))
      next unless serv.items

      serv.items.each do |item|
        item_list = Invoice::ItemList.find_by(item_name: item.description, facility_id: @invoice.facility_id)
        item_list&.update_attributes(ipdsum: (item_list.ipdsum.to_f - item.price.to_f), use_count_ipd: (item_list.use_count_ipd.to_i - 1))
      end
    end
    ...
    # Find Reports
    find_edit_ipd_daily_report

    params[:invoice_ipd][:tax_breakup] = [] if params[:invoice_ipd][:tax_breakup].nil?

    if @invoice.update_attributes(invoice_update_params)
      # to generate for receipt_id for each txn
      Invoice::CreateBillNumberService.call(@invoice)

      @invoice.record_histories.create(user_id: current_user.id, action: 'U', actiontime: Time.current, template_type: 'Invoice:Ipd')

      @invoice_after = @invoice.net_amount
      @adjusted_amount = @invoice.advance_payment

      # Update Reports
      update_ipd_daily_report
      ...
      unless @invoice_after == @invoice_before
        InvoiceLog.create(invoice_date: @invoice.created_at, bkp_bill_number: @invoice.bkp_bill_number, bill_number: @invoice.bill_number, old_total: @invoice_before, new_total: @invoice_after, comment: @invoice.comments, invoice_id: @invoice.id.to_s, user_id: current_user.id, username: current_user.fullname, facility_id: current_facility.id, organisation_id: current_facility.organisation_id, invoice_type: 'IPD')
      end
      # TaxCalculation
      TaxCalculationJob.perform_later(@invoice.id.to_s, 'tax_collected_details') if @invoice.tax_enabled
      Analytics::Financial::FinancialUpdateJob.perform_later(@invoice.id.to_s, 'IPD', @old_amount.to_s)
      ...
      Patients::Summary::TimelineWorker.call(event_name: 'IPD_INVOICE', sub_event_name: 'UPDATED', invoice_id: @invoice.id, user_id: current_user.id, user_name: current_user.fullname, encounter_type: 'IPD', is_draft: @invoice.is_draft)
    end
  end
  ...
  def destroy
    ...
    if @invoice.update(is_canceled: true, refund_amount: @invoice.try(:payment_received), is_refunded: true,
      ...
      if @refund_payment.present?
        update_ipd_refund_payment if @invoice.try(:payment_received).to_f != 0.0
      else
        create_ipd_refund_payment if @invoice.try(:payment_received).to_f != 0.0
      end
      respond_to do |format|
        format.js { render 'invoice/ipd/show' }
      end
      Patients::Summary::TimelineWorker.call(event_name: 'IPD_INVOICE', sub_event_name: 'CANCELLED', invoice_id: @invoice.id, user_id: current_user.id, user_name: current_user.fullname, encounter_type: 'IPD')
    end
  end

  def print_full_invoice
    @number_format = CommonHelper.get_number_format(current_facility, current_organisation)
    @invoice = Invoice::Ipd.find_by(id: params[:invoice_id])
    ...
    respond_to do |format|
      if @invoice.tax_enabled && @invoice_setting.show_tax_in_print
        if params[:page_size] == 'A5'
          format.pdf { render template: 'invoice/ipd/print_full_invoice_tax_enabled.pdf.erb', pdf: 'xyz', layout: 'pdfgen_invoice.html.erb', viewport_size: '1280x1024', page_size: 'A4', disable_smart_shrinking: true, show_as_html: params[:debug].present?, footer: { right: '[page] of [topage]' }, locals: @locals }
        else
          format.pdf { render template: 'invoice/ipd/print_full_invoice_tax_enabled.pdf.erb', pdf: 'xyz', layout: 'pdfgen.html.erb', viewport_size: '1280x1024', page_size: 'A4', disable_smart_shrinking: true, show_as_html: params[:debug].present?, header: { spacing: 4, html: { template: 'layouts/pdf-header.html' } }, footer: { html: { template: 'layouts/pdf-footer.html' }, spacing: 10 }, margin: { left: @print_setting.try(:left_margin).to_i * 10, right: @print_setting.try(:right_margin).to_i * 10, top: @print_setting.try(:header_height).to_i * 10, bottom: @print_setting.try(:footer_height).to_i * 10 }, locals: @locals }
        end
      else
        if params[:page_size] == 'A5'
          format.pdf { render template: 'invoice/ipd/print_full_invoice_tax_disabled.pdf.erb', pdf: 'xyz', layout: 'pdfgen_invoice.html.erb', viewport_size: '1280x1024', page_size: 'A4', disable_smart_shrinking: true, show_as_html: params[:debug].present?, footer: { right: '[page] of [topage]' }, locals: @locals }
        else
          format.pdf { render template: 'invoice/ipd/print_full_invoice_tax_disabled.pdf.erb', pdf: 'xyz', layout: 'pdfgen.html.erb', viewport_size: '1280x1024', page_size: 'A4', disable_smart_shrinking: true, show_as_html: params[:debug].present?, header: { spacing: 4, html: { template: 'layouts/pdf-header.html' } }, footer: { html: { template: 'layouts/pdf-footer.html' }, spacing: 10 }, margin: { left: @print_setting.try(:left_margin).to_i * 10, right: @print_setting.try(:right_margin).to_i * 10, top: @print_setting.try(:header_height).to_i * 10, bottom: @print_setting.try(:footer_height).to_i * 10 }, locals: @locals }
        end
      end
    end
  end
  ...
  def ipd_invoice_params
    params.require(:invoice_ipd).permit(
      ...
      :currency_id, :amount_paid_patient, :patient_id, :admission_id, :tpa_id, :patient_comment, 
      :bill_of_material_breakup, :insurer_group_id, :insurer_id, :is_paid_discounted, :is_free_discounted, 
      :is_paid, :is_free, :is_draft, :converted_to_final_date, :converted_to_final_datetime, 
      :converted_to_final_by, :corporate_id,
      ...
      advance_payment_breakups_attributes: [:advance_payment_id, :advance_display_id, :reason, 
                                            :mode_of_payment, :advance_date, :date, :advance_time, :time, 
                                            :advance_amount, :amount, :currency_symbol, :currency_id, 
      ...
      payment_received_breakups_attributes: [:amount, :currency_symbol, :currency_id, :date, :time, 
                                              :received_by, :received_from, :mode_of_payment, :cash, 
                                              :card, :card_number, :card_transaction_note, 
                                              :paytm_transaction_id, :paytm_transaction_note,
                                              :gpay_transaction_id, :gpay_transaction_note, 
                                              :phonepe_transaction_id, :phonepe_transaction_note, 
                                              :transfer_date, :transfer_note, :cheque_date,
  ...
  def invoice_update_params
    params.require(:invoice_ipd).permit(
      ...
      :other_note, :tax, :additional_discount, :services_discount, :total_discount, 
      :additional_discount_comment, :round, :advance_payment, :amount_remaining,
      :mode_of_payment, :invoice_payment_type, :contact_group_id, :payer_master_id, :credit_type, 
      :tax_enabled, :non_taxable_amount, :payment_received, :payment_pending, :payment_completed, 
      :amount_paid_tpa, :currency_symbol,  :currency_id, :amount_paid_patient, :patient_id, :admission_id, 
      :tpa_id, :patient_comment, :bill_of_material_breakup, :is_paid_discounted, :is_free_discounted, 
      :is_paid, :is_free, :is_draft, :converted_to_final_date, :converted_to_final_datetime, 
      :converted_to_final_by, :amount_received, :offer_on_bill, :offer_on_services, :total_offer, 
      :offer_manager_id, :offer_name, :offer_code, :offer_percentage, :offer_applied_at, 
      :offer_applied_at_name, :offer_applied_by, :offer_applied_by_name, :offer_applied_date, 
      :offer_applied_datetime,
      :consultant_user_id, :service_type, :corporate_id, :tpa_name, :insurance_name, :insurance_id,
      :corporate_name, :dispensary_id, :dispensary_name,
      payment_pending_breakup: [:_id, :amount, :received_from, :date, :time],
      payment_received_breakup: [:_id, :amount, :date, :time, :received_by, :received_from, :received_by,
      ...
      payment_received_breakups_attributes: [:id, :_destroy, :amount, :currency_symbol, :currency_id, 
                                              ...
                                              :phonepe_transaction_id, :phonepe_transaction_note, 
                                              :transfer_date, :transfer_note, :cheque_date, :cheque_note, 
                                              :other_transaction_id, :other_note, :is_settled, :receipt_id,
                                              :amount_received, :has_tax_deducted, :tax_deducted, 
                                              :has_payer_difference, :payer_difference, 
                                              :has_other_revenue_spilage, :other_revenue_spilage, :internal_comment],
  ...
  def patient_params
    params.require(:patient).permit(:fullname, :firstname, :middlename, :lastname, :gender, :age, :age_month, :address_1,
                                    :address_2, :city, :state, :pincode, :email)
  end
  ...
  def fetch_assessment_data
    @ipd_record = Inpatient::IpdRecord.where(admission_id: @admission.id)[0]

    @visit_diagnoses = @ipd_record.try(:diagnosislist)

    @all_visit_procedures = @ipd_record.try(:procedure)

    if @all_visit_procedures.present?
      @visit_procedures = @all_visit_procedures.where(is_performed: true)
    else
      @visit_procedures = nil
    end
