class Inpatient::IpdRecord::OperativeNote::CardiologyNotesController < Inpatient::IpdRecord::OperativeNotesController
  before_action :specialty_params

  def new
    @url_path = "inpatient_ipd_record_operative_note_cardiology_notes_path"
    @procedurenotes = ProcedureNote.where(organisation_id: current_user.organisation_id.to_s, specialty_id: @specialty_id, is_active: true,
  ...
  def edit
    @url_path = "inpatient_ipd_record_operative_note_cardiology_note_path"
    @procedurenotes = ProcedureNote.where(organisation_id: current_user.organisation_id, specialty_id: @specialty_id, is_active: true, '$or' => [{ level: 'organisation' }, { facility_id: current_facility.id, level: 'facility' }, { user_id: current_user.id, level: 'user' }])
    get_advices_list
