module Api
  module V1
    class Inpatient::IpdRecord::WardNotesController < ApiApplicationController
      ...
      def new
        ...
        @url = "/inpatient/ipd_record/ward_note/#{@current_department}_notes"
        @method = "POST"
      ...
      def edit
        @current_department = current_user.department.name.downcase
        @url = "/inpatient/ipd_record/ward_note/#{@current_department}_notes/#{@ipdrecord.id}"
        @method = "PATCH"
        respond_to do |format|
          format.js { render "inpatient/ipd_record/ward_note/form" }
        end
