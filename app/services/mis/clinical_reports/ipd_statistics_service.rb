  ...
  class IpdStatisticsService
    class << self
      def call(mis_params, request = 'json')
        ...
        lists = MisClinical::Ipd::IpdStatistic.collection.aggregate(admission_list_query).to_a || []
      ...
      def ipd_list_data
        @stats_lists.try(:each) do |ipd|
          appointment_date = ipd[:_id].strftime('%d/%m/%Y')
          ipd[:ipd_list].each do |ipd_list|
            male = ipd_list[:gender_stats_fields][:male]
            female = ipd_list[:gender_stats_fields][:female]
            transgender = ipd_list[:gender_stats_fields][:transgender]
            undefined_sex = ipd_list[:gender_stats_fields][:undefined]

            till_twenty = ipd_list[:age_stats_fields][:till_twenty]
            above_twenty_till_forty = ipd_list[:age_stats_fields][:above_twenty_till_forty]
            above_forty_till_sixty = ipd_list[:age_stats_fields][:above_forty_till_sixty]
            above_sixty_till_eighty = ipd_list[:age_stats_fields][:above_sixty_till_eighty]
            above_eighty = ipd_list[:age_stats_fields][:above_eighty]
            undefined = ipd_list[:age_stats_fields][:undefined]
            ...
            if @request == 'json'
              ...
              @data_array << [appointment_date, till_twenty, above_twenty_till_forty, above_forty_till_sixty,
      ...
      def admission_list_query
        Mis::ClinicalService::IpdStatsQueryBuilder.call(@mis_params, @request)
      end
    end
