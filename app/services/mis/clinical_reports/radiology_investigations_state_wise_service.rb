module Mis::ClinicalReports
  class RadiologyInvestigationsStateWiseService
    class << self
      ...
      def advised_appointment_list_data
        @advised_appointment_lists&.each do |ap_list|
          investigation_name = ap_list[:radiology_investigations][:investigationname] || '--'
          status = get_status(ap_list[:radiology_investigations])
          payment_date = if status == 'Declined' && ap_list[:radiology_investigations][:payment_taken_datetime].present?
                           '--'
                         else
                           payment_tdt = ap_list[:radiology_investigations][:payment_taken_datetime]
                           payment_tdt&.in_time_zone&.strftime('%d/%m/%Y') || '--'
                         end
          declined_date = if status == 'Paid' && ap_list[:radiology_investigations][:declined_datetime].present?
                            '--'
                          else
                            declined_dtm = ap_list[:radiology_investigations][:declined_datetime]
                            declined_dtm&.in_time_zone&.strftime('%d/%m/%Y') || '--'
                          end
          performed_dtm =  ap_list[:radiology_investigations][:performed_datetime]
          performed_date = performed_dtm&.in_time_zone&.strftime('%d/%m/%Y') || '--'
          performed_by = if ap_list[:radiology_investigations][:is_performed_outside] == true
                           ap_list[:radiology_investigations][:performed_outside_by] || '--'
                         else
                           ap_list[:radiology_investigations][:performed_by] || '--'
                         end
          patient_name, age, sex, mobile = make_patient_details(ap_list[:patient_id])
          counselled_by = ap_list[:radiology_investigations][:counselled_by] || '--'
          advised_by = ap_list[:radiology_investigations][:advised_by] || '--'
          advised_date = ap_list[:radiology_investigations][:advised_datetime]&.strftime('%d/%m/%Y') || '--'
          age_sex = "#{age}/#{sex}"
      ...
      def match_counselled_date
        {
          "radiology_investigations.counselled_datetime": { "$gte": @mis_params[:start_date].beginning_of_day,
      ...
      def counsellors
        { "radiology_investigations.counselled_by_id": BSON::ObjectId((@mis_params[:counsellor_id]).to_s) }
      end

      def advised_doctors
        { "radiology_investigations.advised_by_id": BSON::ObjectId((@mis_params[:operating_doctor_id]).to_s) }
      end
      ...
      def match_option_for_specific_state(mis_params, match_options)
        ...
        when 'declined'
          match_options.merge("radiology_investigations.has_declined": true)

        when 'payment_taken'
          match_options.merge("radiology_investigations.payment_taken": true)

        when 'agreed'
          match_options[:"radiology_investigations.payment_taken"] = { "$ne": true }
          match_options.merge("radiology_investigations.has_agreed": true)

        when 'performed'
          match_options.merge("radiology_investigations.is_performed": true)

        when 'counselled'
          match_options.merge("radiology_investigations.is_counselled": true)
        else
          match_options
        end
      end
      ...
      def project_options
        {
          ...
          'radiology_investigations.investigationname': 1,
          ...
          'radiology_investigations.payment_taken_datetime': 1,
          'radiology_investigations.declined_datetime': 1,
          'radiology_investigations.performed_datetime': 1,
          'radiology_investigations.counselled_by': 1,
          'radiology_investigations.advised_by': 1,
          'radiology_investigations.performed_by': 1,
          'radiology_investigations.is_performed_outside': 1,
          'radiology_investigations.performed_outside_by': 1,
          'patient_id': 1
        }
      end
    end
