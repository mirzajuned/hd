  ...
  class OpticalConversionSummaryService
    class << self
      ...
      def generate_table_data(patient_presciption)
        ...
        comments = patient_presciption['not_converted_to_converted'] ? '' : patient_presciption[:store_comment]
        ...
        if @request == 'json'
          store_comment = ''
          if comments.present?
            comments.split(',').each { |c| store_comment += "#{c}<br>"}
          end
          ...
          t_data = [
            advised_on, conversion_status, store_comment, patient_name_details, area, patient_address, advised_by_details, converted_by_details, conversion_in_days
           ]
        else
          t_data = [
            advised_on, conversion_status, conversion_remark, comments, patient_id, patient_mrn, patient_name, patient_age_year_only, patient_gender,
            patient_mobilenumber, area, address_1, address_2, city, state, pincode, commune, district, advised_by_name,
            converted_by_name, converted_on, converted_at, conversion_in_days
          ]
        end
