module Mis::ClinicalReports
  class AppointmentDistrictWiseService
    class << self
      def call(mis_params, request = 'json')
        ...
        @data_array << ['DISTRICT', 'COUNT', 'AGE', 'UNDEFINED', 'MALE', 'FEMALE', 'OTHER(GENDER)', 'TYPE'] unless @request == 'json'
        aggregation_query, aggregation_query_count = appointment_list_query

        @district = AppointmentListView.collection.aggregate(aggregation_query).to_a || []
        @district_count = AppointmentListView.collection.aggregate(aggregation_query_count).to_a
        total_records = (@district_count[0][:total_records] if @district_count.count > 0) || 0
      ...
      def appointment_list_data
        @district.each do |location|
          ...
          arr = Mis::ClinicalReports::ClinicalUtility.clinical_location_data(location, href_req, 'district')
          @data_array << arr
          # loop
        end
      end
      ...
      def match_options
        ...
        match_options = match_options.merge(district: { "$nin": ['', nil] })
        match_options
      end
