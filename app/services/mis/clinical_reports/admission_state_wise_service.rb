module Mis::ClinicalReports
  class AdmissionStateWiseService
    class << self
      def call(mis_params, request = 'json')
        ...
        @data_array << ['STATE', 'COUNT', 'AGE', 'UNDEFINED', 'MALE', 'FEMALE', 'OTHER(GENDER)'] unless @request == 'json'

        aggregation_query, aggregation_query_count = appointment_list_query

        @state = AdmissionListView.collection.aggregate(aggregation_query).to_a || []
        @state_count = AdmissionListView.collection.aggregate(aggregation_query_count).to_a
      ...
      def appointment_list_data
        @state.each do |location|
          ...
          arr = Mis::ClinicalReports::ClinicalUtility.clinical_location_data(location, href_req, 'state', 'admission')
          @data_array << arr
          # loop
        end
      end
