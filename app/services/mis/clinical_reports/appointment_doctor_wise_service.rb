...
module Mis::ClinicalReports
  class AppointmentDoctorWiseService
    class << self
      ...
      def clinical_workflow_data
        @clinical_workflows.try(:each) do |clinical_workflow|
          ...
          count = clinical_workflow[:doctor_appointment_count]
      ...
      def clinical_workflow_query
        ...
        aggregation_query = [
          { "$match": match_options },
          { "$project": { _id: 0, doctor_ids: 1 } },
          { "$unwind": '$doctor_ids' },
          { "$group": { _id: '$doctor_ids', doctor_appointment_count: { "$sum": 1 } } },
          { "$sort": { doctor_appointment_count: -1 } }
        ]
