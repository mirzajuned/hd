module Mis::ClinicalReports
  class AdmissionCityWiseService
    class << self
      def call(mis_params, request = 'json')
        ...
        @data_array << ['CITY', 'COUNT', 'AGE', 'UNDEFINED', 'MALE', 'FEMALE', 'OTHER(GENDER)'] unless @request == 'json'

        aggregation_query, aggregation_query_count = appointment_list_query

        @city = AdmissionListView.collection.aggregate(aggregation_query).to_a || []
        @city_count = AdmissionListView.collection.aggregate(aggregation_query_count).to_a
      ...
      def admission_list_data
        @city.each do |location|
          ...
          arr = Mis::ClinicalReports::ClinicalUtility.clinical_location_data(location, href_req, 'city', 'admission')
          @data_array << arr
          # loop
        end
      end
