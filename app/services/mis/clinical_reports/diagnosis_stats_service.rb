    ...
    class << self
      ...
      def diagnosis_list_data
        @stats_lists.try(:each) do |diagnoses_list|
          ...
          diagnosis_date = diagnoses_list[:_id].to_s(:hg_date_format)

          diagnosis_data.group_by{ |diagns| diagns['diagnosis_original_name'] }.each do |diagnosis_name, diagnosis|
            generate_table_data(diagnosis, diagnosis_name, diagnosis_date)
            diagnosis_date = '' if @request == 'json'
          end
        end
        # @data_array
      end
      ...
      def generate_table_data(diagnosis, diagnosis_name, diagnosis_date)
        ...
        ['male', 'female', 'transgender', 'unspecified'].each do |gender|
          ...
          if @request == 'json'
            @data_array << [ diagnosis_date, diagnosis_json_name, gender.titleize, instance_variable_get("@#{gender}_till_fifteen") || 0, instance_variable_get("@#{gender}_above_fifteen_till_fiftyfive") || 0, instance_variable_get("@#{gender}_above_fiftyfive") || 0, instance_variable_get("@#{gender}_undefined") || 0, total ]
            diagnosis_date = diagnosis_json_name = ''
          else
            @data_array << [ diagnosis_date, diagnosis_name&.titleize, diagnosis_code, gender.titleize, instance_variable_get("@#{gender}_till_fifteen") || 0, instance_variable_get("@#{gender}_above_fifteen_till_fiftyfive") || 0, instance_variable_get("@#{gender}_above_fiftyfive") || 0, instance_variable_get("@#{gender}_undefined") || 0, total ]
          end
        end
      end
      ...
      def diagnosis_stats_query
        Mis::ClinicalService::DiagnosisQueryBuilder.call(@mis_params, @request, false)
      end
    end
  end
end
