module Mis::ClinicalReports
  class AppointmentCityWiseService
    class << self
      def call(mis_params, request = 'json')
        ...
        @data_array << ['CITY', 'COUNT', 'AGE', 'UNDEFINED', 'MALE', 'FEMALE', 'OTHER(GENDER)', 'TYPE'] unless @request == 'json'

        aggregation_query, aggregation_query_count = appointment_list_query

        @city = AppointmentListView.collection.aggregate(aggregation_query).to_a || []
        @city_count = AppointmentListView.collection.aggregate(aggregation_query_count).to_a
        total_records = (@city_count[0][:total_records] if @city_count.count > 0) || 0
      ...
      def appointment_list_data
        @city.each do |location|
          next if location[:_id].nil?

          href = @mis_params[:url] + forward_params + back_params
          href_req = [href, @request]
          arr = Mis::ClinicalReports::ClinicalUtility.clinical_location_data(location, href_req, 'city')
          @data_array << arr
          # loop
        end
      end
      ...
      def match_options
        ...
        match_options = match_options.merge(city: { "$nin": ['', nil] })
        match_options
      end
