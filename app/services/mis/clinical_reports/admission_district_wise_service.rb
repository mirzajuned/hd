module Mis::ClinicalReports
  class AdmissionDistrictWiseService
    class << self
      def call(mis_params, request = 'json')
        ...
        @data_array << ['DISTRICT', 'COUNT', 'AGE', 'UNDEFINED', 'MALE', 'FEMALE', 'OTHER(GENDER)'] unless @request == 'json'

        aggregation_query, aggregation_query_count = appointment_list_query

        @district = AdmissionListView.collection.aggregate(aggregation_query).to_a || []
        @district_count = AdmissionListView.collection.aggregate(aggregation_query_count).to_a
      ...
      def appointment_list_data
        @district.each do |location|
          ...
          arr = Mis::ClinicalReports::ClinicalUtility.clinical_location_data(location, href_req, 'district', 'admission')
          @data_array << arr
          # loop
        end
      end
