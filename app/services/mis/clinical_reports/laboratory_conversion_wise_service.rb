module Mis::ClinicalReports
  class   LaboratoryConversionWiseService
    class << self
      ...
      def investigation_appointment_data
        query, count_query = investigation_appointment_query
        @investigation_appointment_list = CaseSheet.collection.aggregate(query).to_a || []
        aggregation_query_count = CaseSheet.collection.aggregate(count_query).to_a || []
      ...
      def make_anchor_url(status_wise_count, appointment_date)
        ...
        if @request == 'json'
          ...
          agreed_count, declined_count, paid_count, performed_count, advised_count = anchored_count(status_wise_count,
                                                                                                    href)
        end
        [advised_count, declined_count, performed_count, paid_count, agreed_count]
      end

      def anchored_count(status_wise_count, href)
        agreed_count, declined_count, paid_count, performed_count, advised_count = status_wise_count
        if advised_count > 0
        ...
        [agreed_count, declined_count, paid_count, performed_count, advised_count]
      end
      ...
      def group_options
        { _id: '$dayMonthyearUTC', investigation_state_list: {
          "$push":
          {
            is_advised: '$laboratory_investigations.is_advised',
            has_agreed: '$laboratory_investigations.has_agreed',
            has_declined: '$laboratory_investigations.has_declined',
            payment_taken: '$laboratory_investigations.payment_taken',
      ...
      def project_options
        {
          dayMonthyearUTC: {
            '$dateToString': { format: '%d-%m-%Y', date: '$laboratory_investigations.advised_datetime' }
          },
