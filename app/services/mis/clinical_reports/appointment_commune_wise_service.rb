module Mis::ClinicalReports
  class AppointmentCommuneWiseService
    class << self
      def call(mis_params, request = 'json')
        ...
        @data_array << ['COMMUNE', 'COUNT', 'AGE', 'UNDEFINED', 'MALE', 'FEMALE', 'OTHER(GENDER)', 'TYPE'] unless @request == 'json'

        aggregation_query, aggregation_query_count = appointment_list_query

        @commune = AppointmentListView.collection.aggregate(aggregation_query).to_a || []
        @commune_count = AppointmentListView.collection.aggregate(aggregation_query_count).to_a
      ...
      def appointment_list_data
        @commune.each do |location|
          ...
          arr = Mis::ClinicalReports::ClinicalUtility.clinical_location_data(location, href_req, 'commune')
          @data_array << arr
          # loop
        end
      end
