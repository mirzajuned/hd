module Mis::ClinicalReports
  class AppointmentPaymentTypeWiseService
    class << self
      ...
      def forward_params(type)
        ...
        category = "&appointment_category_label=#{type}"
        has_params = '&has_params=true'

        (start_date + end_date + time_period + organisation_id + facility_id + facility_name + group + title + category + has_params)
      end
      ...
      def appointment_list_query
        ...
        aggregation_query = [
          { "$match": match_options },
          { "$group": { _id: '$appointment_category_label', appointment_count: { "$sum": 1 } } },
