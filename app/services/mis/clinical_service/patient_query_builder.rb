  ...
  class PatientQueryBuilder
    class << self
      def call(mis_params, request)
        ...
        aggregation_query = [
          { "$match": match_options },
          { "$sort": { 'filtering_fields.appointment_start_time': -1 } }
        ]
      ...
      def match_options
        ...
        # Date
        # match_options = match_options.merge('filtering_fields.appointment_date': { "$gte": @mis_params[:start_date], "$lte": @mis_params[:end_date] })
        match_options = match_options.merge!('filtering_fields.appointment_start_time': { "$gte": @mis_params[:start_date].beginning_of_day, "$lte": @mis_params[:end_date].end_of_day })
        #
        # Patient Type
        ...
        if @mis_params[:referral_source].present?
          match_options = match_options.merge!('appointment_display_fields.referral_type': @mis_params[:referral_source])
        end
        ...
        elsif gender.present? && gender == 'undefined'
