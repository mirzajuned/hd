      ...
      def call(start_date, end_date, org_id = nil)
        ...
        facilities.each do |facility|
          ...
          date_wise_list.each do |val|
            ...
            admission_date = key
            city = value.group_by { |al| al[:city].to_s }
            state = value.group_by { |al| al[:state].to_s }
            commune = value.group_by { |al| al[:commune].to_s }
            district = value.group_by { |al| al[:district].to_s }
            ...
            common_arr.each do |loc|
              loc[0].each do |k, data|
                ...
                location_statistics = MisClinical::Ipd::LocationStatistic.find_or_create_by(
                  organisation_id: organisation_id, facility_id: facility_id,
                  admission_date: admission_date, type: location_type, location_name: location_name
                )
