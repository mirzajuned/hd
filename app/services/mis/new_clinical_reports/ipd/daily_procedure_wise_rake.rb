          ...
          (start_date.to_date..end_date.to_date).each do |date|
            if total_advised_list.present?
              ...
              if total_advised.present? && total_advised[0][:lists].present?
                ...
                grp_by_advised_procedure = advised_date_list.group_by { |al| al[:procedurename].to_s }
                grp_by_performed_procedure_from_advised = advised_date_list.select { |proc| proc[:is_advised] == true && proc[:is_performed] == true }.group_by { |al| al[:procedurename].to_s }
              end
            end
            if total_performed_list.present?
              ...
              if total_performed.present? && total_performed[0][:lists].present?
                ...
                grp_by_performed_procedure_from_performed = performed_date_list.select { |proc| proc[:is_advised] == true && proc[:is_performed] == true }.group_by { |al| al[:procedurename].to_s }
              end
            end
            ...
            loop_in_keys.each do |procedure|
              ...
              procedure_details = MisClinical::Ipd::DailyProcedureConversion.find_or_create_by(
                organisation_id: organisation_id, facility_id: facility_id,
                procedure_date: date, procedure_name: procedure
              )
              procedure_details.procedure_name = procedure
