    ...
    class << self
      ...
      def generate_table_data(invoice)
        ...
        if @request == 'json'
          ...
          t_data = [invoice_data['department_name'], bill_details, patient_details, area, 
                    location, i_type, payer_details, customer_comment, internal_comment, 
                    item_offer, additional_offer, total_offer, item_discount, additional_discount, 
                    total_discount, invoice_data['total'], invoice_data['tax'], 
                    invoice_data['gross_amount'], still_pending.round(2), pending_after_cancel, 
                    amount_adjusted, invoice_data['advance_payment'], payment_received, 
                    tax_deducted, spillage, payer_difference, total_settled, refund_amount, 
                    refund_count, invoice_data['no_of_servies'], invoice_data['no_of_receipts'], 
                    user_details, invoice_modified, bill_status, refund_history_json] 
        else
          t_data = [invoice_data['department_name'], bill_no, bill_date, patient_name, patient_age,
                    patient_gender, patient_identifier_id, patient_mrn, area, city, state, 
                    district, commune, pincode, i_type, payer_details, customer_comment, 
                    internal_comment, item_offer, additional_offer, total_offer, item_discount,
                    additional_discount, total_discount, invoice_data['total'], invoice_data['tax'],
                    invoice_data['gross_amount'], still_pending.round(2), pending_after_cancel, 
                    amount_adjusted, invoice_data['advance_payment'], payment_received, 
                    tax_deducted, spillage, payer_difference, total_settled, refund_amount, 
                    refund_count, invoice_data['no_of_servies'], invoice_data['no_of_receipts'], 
                    user_name, user_designation, modified_username, modified_date, bill_status, 
                    reason]
        end
