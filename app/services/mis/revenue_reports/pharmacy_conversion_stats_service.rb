...
module Mis::RevenueReports
  class PharmacyConversionStatsService
    class << self
      def call(mis_params, request = 'json')
        ...
        @prescriptions = if @request == 'xls'
            prescriptions || []
          else
            prescriptions[0][:data] || []
          end

        total_records = if prescriptions.present? && @request == 'xls'
      ...
      def forward_params(facility_id, facility_name, advised_date, store_id, store_name, is_converted = nil)
        ...
        params_str += "&pharmacy_store_id=#{store_id}"
        params_str += "&pharmacy_store_name=#{store_name}"
        params_str += "&conversion_status=#{is_converted}" if is_converted.present?
        params_str += '&group=finance'
        params_str += '&title=pharmacy_conversion_summary'
      ...
      def back_params(store_id, store_name)
        ...
        back_params_str += "&back_pharmacy_store_id=#{store_id}"
        back_params_str += "&back_pharmacy_store_name=#{store_name}"
