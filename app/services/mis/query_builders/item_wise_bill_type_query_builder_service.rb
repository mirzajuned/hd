module Mis::QueryBuilders
  class ItemWiseBillTypeQueryBuilderService
    class << self
      ...
      def revenue_report_match
        ...
        # Date/Time
        match_options[:item_entry_date] = { "$gte": @mis_params[:start_date].beginning_of_day,
      ...
      def default_report_array
        @detailed_reports = ['']
        @stats_reports = ['procedure_bill_type_stats']
      end
      # EOF default_report_array

      def default_condition(report_name)
        match_hash = { service_type: {"$in": ['procedure', 'Procedure']} }
        match_hash
      end
