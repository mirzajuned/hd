class AdmissionListViews::UpdateService
  class << self
    def call(admission_list_view, admission)
    ...
    def get_ward_details(admission)
      if admission.ward_id.nil? || admission.room_id.nil? || admission.bed_id.nil?
        ...
        { ward_code: ward.code, ward_name: ward.name, room_code: room.code, room_name: room.name, bed_code: bed.try(:code), bed_name: bed.try(:name) }
      end
    end
  end
end
