...
class ExpenseReceiptUploader < ApplicationAssetUploader
  # Override the directory where uploaded files will be stored.
  # This is a sensible default for uploaders that are meant to be mounted:
  def store_dir
    'expense-reciept-assets'
  end
  ...
  def filename
    "#{@model.id}_#{@model.id}_photo_#{Date.today.strftime('%Y%m%d')}_#{Time.current.strftime('%H%M%S')}.#{file.extension}" if original_filename
  end
