...
class AppointmentAssetUploader < ApplicationAssetUploader
  ...
  def filename
    "#{@model.appointment.id}_photo_#{Date.current.strftime('%Y%m%d')}_#{Time.current.strftime('%H%M%S')}.#{file.extension}" if original_filename
  end
end
