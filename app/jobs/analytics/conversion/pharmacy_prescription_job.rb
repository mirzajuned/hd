class Analytics::Conversion::PharmacyPrescriptionJob < ActiveJob::Base
  queue_as :urgent
  DATA_CREATION_ARRAY = ['day', 'week', 'month', 'year'].freeze
  def perform(prescription_id, _current_user_id, mode, prev_state = '')
    if ['Clinical', 'Pharmacy', 'ClinicalTrue', 'ClinicalFalse'].include?(mode)
    ...
    if prescription.converted.nil?
      DATA_CREATION_ARRAY.each_with_index do |type, _indx|
        ...
        conversion = Analytics::Conversion::PharmacyPrescription.find_by(
        ...
        ) || Analytics::Conversion::PharmacyPrescription.new
        ...
        if conversion.new_record?
          ...
          conversion.department_id = '284748001'
      ...
      if prescription.converted == true
        DATA_CREATION_ARRAY.each_with_index do |type, _indx|
          ...
          doctor_conversion = Analytics::Conversion::PharmacyPrescription.find_by(
            organisation_id: organisation_id, facility_id: facility_id, specialty_id: specialty_id, user_id: user.id,
            data_type: type, data_type_range: hashed_data[type], in_year: start_date.year
          )
          conversion = Analytics::Conversion::PharmacyPrescription.find_by(
            organisation_id: organisation_id, facility_id: facility_id, specialty_id: specialty_id, data_type: type,
            data_type_range: hashed_data[type], department_id: '284748001', in_year: start_date.year
          )
          invoice = Invoice::Inventories::Department::PharmacyInvoice.find_by(prescription_id: prescription.id)
        ...
        unless prev_state.nil?
          DATA_CREATION_ARRAY.each_with_index do |type, _indx|
            ...
            conversion = Analytics::Conversion::PharmacyPrescription.find_by(
              ...
              department_id: '284748001', data_type: type, data_type_range: hashed_data[type],
