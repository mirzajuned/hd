class RadiologyInvestigation
  ...
  # method commented because of ActionView::Template::Error (wrong number of arguments (0 for 1)) issue when accessing this model data from record model which is embedded in patient-opd
  # def radiologyoptions_collection(collection)
  #   write_attribute(:radiologyoptions_collection, collection.reject(&:blank?)) #same version as, write_attribute(:radiologyoptions_collection, collection.map{|v| v.blank? ? nil : v}.compact)
  # end
end
